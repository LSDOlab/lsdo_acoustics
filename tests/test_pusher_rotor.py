...
file_name = GEOMETRY_PATH / 'pusher_rotor.stp'
...
# PROJECTIONS
p11 = rotor_disk.project(np.array([5.000, 1.000, 0.000]), direction=np.array([1.,0.,0.]), plot=False)
p12 = rotor_disk.project(np.array([5.000, -1.000, 0.000]), direction=np.array([1.,0.,0.]), plot=False)
p21 = rotor_disk.project(np.array([5.000, 0.000, 1.000]), direction=np.array([1.,0.,0.]), plot=False)
p22 = rotor_disk.project(np.array([5.000, 0.000, -1.000]), direction=np.array([1.,0.,0.]), plot=False)
rotor_in_plane_x = am.subtract(p12,p11)
rotor_in_plane_y = am.subtract(p21,p22)
rotor_origin = rotor_disk.project(np.array([5.000, 0.000, 0.000]), direction=np.array([1.,0.,0.]))
...
# PROJECTIONS
le_tip = np.array([4.985, 1.000, -0.063])
le_root = np.array([4.971, 0.200, -0.027])
te_tip = np.array([5.015, 1.000, 0.063])
te_root = np.array([5.029, 0.200, 0.027])
...
p_le = rotor_blade.project(blade_le, direction=np.array([1., 0., 0.]), grid_search_n=50, plot=False) # projection onto LE of BLADE
p_te = rotor_blade.project(blade_te, direction=np.array([-1., 0., 0.]), grid_search_n=50, plot=False) # projection onto TE of BLADE

rotor_blade_chord = am.subtract(p_le, p_te) # produces array map of vectors radially, from TE to LE

rotor_disk_le_proj = rotor_disk.project(p_le.evaluate(), direction=np.array([1., 0., 0.]), grid_search_n=50, plot=False) # LE projected onto disk
rotor_disk_te_proj = rotor_disk.project(p_te.evaluate(), direction=np.array([-1., 0., 0.]), grid_search_n=50, plot=False) # TE projected onto disk
...
if memory_debug:
    ...
    sim = Simulator(caddee_csdl_model, analytics=True, display_scripts=True, name='pusher_rotor')
    sim.run()
    # sim.check_totals()
